...
class PDFTableExtractor:
    ...
    # def _format_for_csv(self, df: pd.DataFrame, info: Dict[str, Any]) -> pd.DataFrame:
    #     """
    #     Format dataframe to match required CSV structure.
        
    #     Args:
    #         df: Cleaned dataframe
    #         info: File information containing parsed filename data
            
    #     Returns:
    #         pd.DataFrame: Formatted dataframe ready for CSV
    #     """
    #     if df.empty:
    #         return df
    ...
    #     formatted_df['作業日'] = [formatted_date] * len(df)
    ...
    #     # 🔥 NEW: Smart datetime formatting with midnight crossover detection
    #     start_times = []
    #     end_times = []
    ...
    #                 if start_hour >= 22 and end_hour <= 6:  # e.g., 22:35 → 01:37 or 23:45 → 00:20
    #                     # End time is next day
    #                     start_times.append(f"{formatted_date} {start_time}")
    #                     end_times.append(f"{next_date} {end_time}")
    #                 else:
    #                     # Same day
    #                     start_times.append(f"{formatted_date} {start_time}")
    #                     end_times.append(f"{formatted_date} {end_time}")
                        
    #             except (ValueError, IndexError):
    #                 # If parsing fails, use same date for both
    #                 start_times.append(f"{formatted_date} {start_time}" if start_time else '')
    #                 end_times.append(f"{formatted_date} {end_time}" if end_time else '')
    #         else:
    #             # Handle empty times
    #             start_times.append(f"{formatted_date} {start_time}" if start_time else '')
    #             end_times.append(f"{formatted_date} {end_time}" if end_time else '')
